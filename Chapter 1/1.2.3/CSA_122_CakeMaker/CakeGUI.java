 ...
 * graphics libraries, inner classes, image IO, Exception, and
...
public class CakeGUI {
            ...
            public void paintComponent(Graphics g) {
                ...
                if (CakeGUI.READY_TO_EXECUTE) {
                    for (CakeObject cake : cakes) {
                        System.out.println("Cake object: " + cake); // Standard output will show a list of all cakes
                                                                    // with each repainting.
                        if (cake.hasCandles()) {
                            if (cake.getTiers() == 2) {
                                g.drawImage(twoTierCakeCandlesImage, cake.getX(), cake.getY(), cake.getWidth(),
                                        cake.getHeight(), null);
                            } else if (cake.getTiers() > 2) {
                                g.drawImage(threeTierCakeCandlesImage, cake.getX(), cake.getY(), cake.getWidth(),
                                        cake.getHeight(), null);
                            } else {
                                g.drawImage(cakeCandlesImage, cake.getX(), cake.getY(), cake.getWidth(),
                                        cake.getHeight(), null);
                            }
                        } else {
                            if (cake.getTiers() == 2) {
                                g.drawImage(twoTierCakeImage, cake.getX(), cake.getY(), cake.getWidth(),
                                        cake.getHeight(), null);
                            } else if (cake.getTiers() > 2) {
                                g.drawImage(threeTierCakeImage, cake.getX(), cake.getY(), cake.getWidth(),
                                        cake.getHeight(), null);
                            } else {
                                g.drawImage(cakeImage, cake.getX(), cake.getY(), cake.getWidth(), cake.getHeight(),
                                        null);
                            }
                        }
                    }
                            ...
                            if (methodArgs != null) // null args means illegal or undefined method call, ignore it
                            {
                                if (methodArgs[0].equals("")) {
                                    ...
                                    try {
                                        ...
                                        if (cakeMethodIdentifier.equals("setX") && valueToSet >= CakeObject.MIN_X_VALUE
                                                && valueToSet <= CakeObject.MAX_X_VALUE) {
                                            cake.setX(valueToSet);
                                        }
                                        if (cakeMethodIdentifier.equals("setY") && valueToSet >= CakeObject.MIN_Y_VALUE
                                                && valueToSet <= CakeObject.MAX_Y_VALUE) {
                                        ...
                                        System.out.println("Trying alphabetical methods with actual params : \""
                                                + valueToSet + "\"");
                                        // implement methods with a string param (none in this version )
                                        /*
                                         * if (cakeMethodIdentifier.equals("setColor") && valueToSet.equals("Blue") ||
                                         * valueToSet.equals("Red"))
                                         * {
    ...
    private void loadFiles() {
        ...
        try {
            ...
            System.err.println("Warning: Could not open file kitchen.png");
        }
        try {
            cakeImage = ImageIO.read(new File("oneTierCake.png"));
        } catch (Exception e) {
            System.err.println("Warning: Could not open file oneTierCake.png");
        }
        try {
            twoTierCakeImage = ImageIO.read(new File("twoTierCake.png"));
        } catch (Exception e) {
            System.err.println("Warning: Could not open file twoTierCake.png");
        }
        try {
            threeTierCakeImage = ImageIO.read(new File("threeTierCake.png"));
        } catch (Exception e) {
            System.err.println("Warning: Could not open file threeTierCake.png");
        }
        try {
            cakeCandlesImage = ImageIO.read(new File("oneTierCakeCandles.png"));
        } catch (Exception e) {
            System.err.println("Warning: Could not open file oneTierCakeCandles.png");
        }
        try {
            twoTierCakeCandlesImage = ImageIO.read(new File("twoTierCakeCandles.png"));
        } catch (Exception e) {
            System.err.println("Warning: Could not open file twoTierCakeCandles.png");
        }
        try {
            threeTierCakeCandlesImage = ImageIO.read(new File("threeTierCakeCandles.png"));
        } catch (Exception e) {
            System.err.println("Warning:Could not open file threeTierCakeCandles.png");
        }
    }
}