 ...
 * class methods like equals, split, and substring that could be good to
 * examine.
 * 
 */
    ...
    private CakeAction checkConstructors(String[] tokens) {
        if (tokens.length >= 5) {
            ...
            try {
                ...
                System.err.println("CodeCompiler: could not process " + Arrays.toString(tokens) + "\n");
            }
        }
        return null;
    }
                ...
                for (Method method : Method.values()) {
                    if (methodIdentifier.equals(method.getMethodIdentifier())) {
                        ...
                        if (tokens[tokens.length - 1].substring(tokens[tokens.length - 1].length() - 2).equals(");")) {
                            ...
                            // no arg method call OR match # of args provided with # of params defined (no
                            // overloading)
